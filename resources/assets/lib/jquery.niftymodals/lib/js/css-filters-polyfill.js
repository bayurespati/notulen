	...
	var polyfilter = {
			...
			window.setTimeout(function(){
				...
				xmlHttpCheck.onreadystatechange = function(){
					if(xmlHttp.readyState == 4 && xmlHttp.status != 200){
						alert('The configured path \r\rvar polyfilter_scriptpath = "' + window.polyfilter_scriptpath + '"\r\rseems wrong!\r\rConfigure the polyfill\'s correct absolute(!) script path before referencing the css-filter-polyfill.js, like so:\r\rvar polyfilter_scriptpath = "/js/css-filter-polyfill/";\r\rLeaving IE dead in the water is no option. You damn Mac user... ;)');
					}
				};
				try{
		...
		scriptpath:		
			window.polyfilter_scriptpath ? window.polyfilter_scriptpath : (function(){
				alert('Please configure the polyfill\'s absolute(!) script path before referencing the css-filter-polyfill.js, like so:\r\nvar polyfilter_scriptpath = "/js/css-filter-polyfill/";');
				return './'
			})(),
